...
function ApiCallAndStoreInLocalStorage() {
  fetch("https://activity-utuvyi37ea-ew.a.run.app/v5/activity/")
    ...
    .then((data) => {
      ...
      data.forEach((activity) => {
        ...
        if (
          activity.label !== null ||
          (activity.start !== null && activity.end !== null) ||
...
if (ApiTargetContainer) {
    ApiCallAndStoreInLocalStorage();
} else {
  console.info(
    "INFO : en affichage seul ce script fonctionne via l'id='ApiTargetContainer'et il n'a pas été trouvé dans le document HTML."
  );
}
